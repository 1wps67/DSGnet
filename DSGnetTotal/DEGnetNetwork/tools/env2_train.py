...
class trainData(Dataset):
    def __init__(self, img_path, txt_path,vis_keypoint_path,what_do, data_transforms=None):
        with open(os.path.join(txt_path, 'label.txt')) as input_file:
            # 以list形式保存txt文件的每一行
            lines = input_file.readlines()
            self.img_path = img_path
            self.img_label = txt_path
            self.img_viskeypoints_path = vis_keypoint_path
    ...
    def __getitem__(self, item):
        ...
        img_viskeypoints_path = os.path.join(self.img_viskeypoints_path,img_name.split('/')[-1][0:-11] + '.jpg')
        vis_keypoint_pic = cv2.imread(img_viskeypoints_path,cv2.IMREAD_GRAYSCALE)
        vis_keypoint_pic = np.array(vis_keypoint_pic)
        vis_keypoint_pic = np.expand_dims(vis_keypoint_pic,axis = -1)
        '''
        ...
        if self.data_transforms is not None:
            for transforms in self.data_transforms:
                vis_keypoint_pic = transforms(vis_keypoint_pic)

        return img, vis_keypoint_pic ,img_label
...
def main():
    ...
    train_vis_keypoint_path = r'/mnt/lwp/mmdetection_final_clean/vis_keypoints'
    test_vis_leypoint_path = r'/mnt/lwp/mmdetection_final_clean_test/vis_keypoints'
    ...
    # 构造训练集
    train_dataset = trainData(train_pic_path, train_label_path,train_vis_keypoint_path, 'train', transforms_list)
    ...
    # 构造测试集
    val_dataset = trainData(val_pic_path, val_label_path,test_vis_leypoint_path,'val', transforms_list)
